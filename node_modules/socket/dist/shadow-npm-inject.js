...
function findPackageNodes(tree, name, version) {
  ...
  while (queue.length) {
    ...
    if (!targetVersion && Array.isArray(node.errors) && node.errors.length) {
      require$$8.debugFn('notice', `miss: version for ${node.name} due to errors:\n`, node.errors);
    }
...
async function getAlertsMapFromArborist(arb, options) {
  const opts = {
    ...
    nothrow: false,
    ...options,
    filter: utils.toFilterConfig(require$$9.getOwn(options, 'filter'))
  };
...
function getDetailsFromDiff(diff, options) {
  ...
  if (!diff) {
    require$$8.debugFn('notice', `miss: diff is ${diff}`);
    return details;
  }
...
function updateNode(node, newVersion, newVersionPackument) {
  ...
  if (require$$9.hasOwn(newVersionPackument, 'deprecated')) {
  ...
  if (oldDeps) {
    for (const oldDepName of Object.keys(oldDeps)) {
      if (!require$$9.hasOwn(newDeps, oldDepName)) {
  ...
  for (const newDepName of Object.keys(newDeps)) {
    if (!require$$9.hasOwn(oldDeps, newDepName)) {
...
function updatePackageJsonFromNode(editablePkgJson, tree, node, newVersion, rangeStyle) {
  ...
  for (const depField of ['dependencies', 'optionalDependencies', 'peerDependencies']) {
    ...
    const depValue = require$$9.hasOwn(depObject, name) ? depObject[name] : undefined;
...
//# debugId=e0cb2c5f-febd-4252-b159-07a2e42a304a
